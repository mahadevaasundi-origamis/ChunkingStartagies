...
def create_sections(
	...
	if blob_name.lower().endswith((".jpg", ".png", ".jpeg")):
		...
		try:
			...
			# Initialize SpacyTextSplitter with appropriate parameters
			...
			# Process each page and maintain page tracking
			current_chunk = []
			current_pages = set()
			current_length = 0
			...
			for page_num, _, text in page_map:
				...
				for sentence in sentences:
					...
					# If adding this sentence would exceed the threshold
					if current_length + sentence_length > base_threshold and current_chunk:
						...
						current_pages = {end_page}  # Keep only the last page for overlap
			...
			queue_logger.error(f"Error processing file '{blob_name}' with SpacyTextSplitter: {str(e)}")
			return []
