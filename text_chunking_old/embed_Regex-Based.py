...
async def startEmbedding(file_name, email,comp_id,role):
	...
	"""
      1. Added RecursiveCharacterTextSplitter from LangChain for efficient text splitting
         Implemented comprehensive regex patterns specifically for PDF content:
		Headers (both numbered and unnumbered)
		Section breaks (horizontal lines, double lines)
	  2.Paragraph breaks
		Lists (numbered and bulleted)
		Tables
		Page breaks
		Common PDF artifacts (page numbers, copyright notices)
	  4.Improved text processing:
		Better text cleaning and normalization
		Accurate page range tracking
		Proper handling of multi-page chunks
		The new implementation:
			Combines all text while maintaining page position tracking
			Uses regex patterns to identify natural break points in PDF content
			Splits text into chunks while respecting:
			Document structure (headers, sections, etc.)
			Size limits (base_threshold)
			Overlap requirements (buffer_percent)
			Maintains accurate page range information for each chunk
			Key benefits:
			Better preservation of document structure
			More natural chunk boundaries
			Improved handling of PDF-specific elements
			Maintains semantic coherence within chunks
			Would you like me to make any adjustments to this implementation? For example, we could:
			Add more regex patterns for specific document types
			Adjust the pattern priorities
			Modify the chunk size handling
			Add additional PDF-specific cleaning steps
