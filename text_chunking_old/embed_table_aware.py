...
def is_table(content):
    table_patterns = [
        r'(\+-{3,}\+.*){2,}',  # ASCII tables
        r'(\|.*\|\n){2,}',      # Markdown tables (at least 2 rows)
        r'<table>'              # HTML tables
    ]
    return any(re.search(pattern, content) for pattern in table_patterns)

def table_aware_chunking(text, max_size=10000):
    chunks = []
    current_chunk = ""
    in_table = False
    current_table = ""
    ...
    for line in text.split('\n'):
        if is_table(line) or in_table:
            in_table = True
            current_table += line + '\n'
            
            # Check if table ended
            if '</table>' in line or ('+-' in line and current_table.count('+-') >= 2):
                if len(current_chunk) + len(current_table) <= max_size:
                    current_chunk += current_table
                else:
                    if current_chunk:
...
def table_to_html(table):
    """Enhanced HTML table representation with structure/metadata preservation"""
    table_html = "<table>"
    
    # Extract and sort cells by row and column
    rows = []
    for i in range(table.row_count):
        row_cells = [cell for cell in table.cells if cell.row_index == i]
        rows.append(sorted(row_cells, key=lambda cell: cell.column_index))
    ...
    for row_cells in rows:
        ...
        for cell in row_cells:
            ...
            # Add class for header cells
            cell_class = " class='header-cell'" if tag == "th" else ""
            
            table_html += f"<{tag}{cell_spans}{cell_class}>{html.escape(cell.content)}</{tag}>"
        table_html += "</tr>"
    
    table_html += "</table>"
    return table_html

def extract_tables_and_text(page, tables_on_page):
    """Extract content with positional metadata for tables"""
    ...
    # Build content with tables and text
    content_parts = []
    current_text = ""
    last_table_id = -1
    ...
    for idx, table_id in enumerate(table_chars):
        if table_id == -1:
            current_text += page.content[page_offset + idx]
        else:
            if current_text:
                content_parts.append(('text', current_text))
                current_text = ""
            
            if table_id != last_table_id:
                content_parts.append(('table', tables_on_page[table_id]))
...
def create_table_aware_sections(
    ...
    upper_limit = int(base_threshold * (1 + buffer_percent/100))
    lower_limit = int(base_threshold * (1 - buffer_percent/100))
    chunk_id_prefix = re.sub(r'[ .]', '_', blob_name)
    input_data = []
    
    # Normalize page_map to (page_num, page_text) format
    normalized_page_map = []
    for item in page_map:
        # Handle both (page_num, page_text) and (page_num, something_else, page_text) formats
        if isinstance(item, (list, tuple)):
            if len(item) == 3:
                page_num, _, page_text = item  # Ignore middle value
            elif len(item) == 2:
                page_num, page_text = item
            else:
                raise ValueError("page_map items must be tuples of length 2 or 3")
        else:
            raise ValueError("page_map items must be tuples or lists")
            
        normalized_page_map.append((page_num, str(page_text)))  # Ensure page_text is string
    
    # Build character-to-page index
    char_page_map = []
    current_offset = 0
    for page_num, page_text in normalized_page_map:
        page_length = len(page_text)
        char_page_map.extend([page_num] * page_length)
        current_offset += page_length
    
    # Combine all pages into single text
    full_text = "\n".join(page_text for _, page_text in normalized_page_map)
    chunks = table_aware_chunking(full_text, upper_limit)
    ...
    for chunk_idx, chunk in enumerate(chunks):
        ...
        start_char = full_text.find(chunk, current_pos)
        if start_char == -1:
            start_char = current_pos
        end_char = start_char + chunk_length
        
        try:
            start_page = char_page_map[start_char]
            end_page = char_page_map[end_char-1]
        except IndexError:
            start_page = char_page_map[0] if char_page_map else 1
            end_page = char_page_map[-1] if char_page_map else 1
        
        page_range = f"{start_page}-{end_page}" if start_page != end_page else str(start_page)
        current_pos = end_char
        
        # Check for table content
        has_tables = is_table(chunk)
    ...
    # Add sentence overlap between text chunks
    if overlap_sent_count > 0:
        for i in range(1, len(input_data)):
            prev = input_data[i-1]
            curr = input_data[i]
            
            if not prev['has_tables'] and not curr['has_tables']:
                prev_sentences = split_into_sentences(prev['content'])
                overlap = prev_sentences[-overlap_sent_count:]
                
                if overlap and not curr['content'].startswith(" ".join(overlap)):
                    curr['content'] = " ".join(overlap) + " " + curr['content']
