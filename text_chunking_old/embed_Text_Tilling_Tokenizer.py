...
def create_sections(
	...
	if blob_name.lower().endswith((".jpg", ".png", ".jpeg")):
		...
		try:
			...
			tt = TextTilingTokenizer(w=20, k=10)  # w=window size, k=number of tokens per block
			
			# Split the text into chunks based on semantic boundaries
			...
			queue_logger.error(f"Error processing file '{blob_name}' with TextTilingTokenizer: {str(e)}")
			return []
